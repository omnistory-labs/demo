$(document).ready(() => {
  ...
  const toastCall = document.getElementById("ToastCall");
  const toast = new bootstrap.Toast(toastCall);
  let sessionId;
  const callTimer = new Object();

  document.getElementById("bell2").volume = 0.4;
  ...
  omnitalk.onmessage = async (evt) => {
    ...
    switch (evt.cmd) {
      ...
      case "RINGING_EVENT":
        ...
        $("#answerBtn").attr("data-session", evt.session);
        $("#hangupBtn").attr("data-session", evt.session);
        toast.show();
        break;
      case "BROADCASTING_EVENT":
        if (evt.roomtype == "audiocall" || evt.roomtype == "audioroom") {
          $("#bell").trigger("play");
          addParti(evt.user_id, evt.user_name);
        } else {
      ...
      case "ACCEPT_EVENT":
        toast.hide();
        $("#bell").trigger("play");
        addParti(evt.user_id, evt.caller);
        break;
      case "REJECT_EVENT":
        toast.hide();
        break;
  ...
  async function regiRoomFunc() {
    ...
    // Make SIP Number
    const number = await omnitalk.makeSipnum();
    console.log(`Make sip number: ${number}`);
  ...
  $("#hangupBtn").click(async () => {
    const peerSessionId = $("#hangupBtn").attr("data-session");
    $("#bell2").trigger("pause");
    toast.hide();
    await omnitalk.leave(peerSessionId);
  });
  ...
  $(document).on("click", ".JoinBtn", async function () {
    ...
    if (check == true && $(secretClass).val() == undefined) {
      let room =
        '<div class="form-box reg-box secret-box" style="padding:10px;">';
      room += '<div class="box-input-wrap">';
      room += `<input type="password" maxLength="6" placeholder="secret" id="secret-${index}">`;
      room += `<button type="button" class="btn button-default btn-sm JoinBtn" data-check=${check} data-index=${index} data-room_id=${roomId}>Join</Button>`;
      room += "</div>";
      room += "</div>";
      ...
      if (join.result != "success") {
        ...
        partilist.map((item, index) => {
          if (item.room_type == "audiocall" || item.room_type == "audioroom") {
            addParti(item.user_id, item.user_name);
          } else {
        ...
        if (join.call_number) {
          $("#sipNum").attr("value", join.call_number);
        }
      }
    }
  });
  ...
  function addParti(user_id, user_name) {
    ...
    parti += `<span style="font-size:0.8rem">${user_name}</span>`;
    parti += "</div>";
    parti += "</div>";
    parti += "</div>";
  ...
  const init = (async () => {
    ...
    roomlist.map((item, index) => {
      let roomType;
      if (item.sip_support == false) return;
      ...
      room += "<div class='card mb-3' style='max-width: 18rem;'>";
      ...
      room += `<button type="button" class="btn button-default btn-sm JoinBtn" data-check=${item.secret} data-index=${index} data-room_id=${item.room_id}>Join</button>`;
      room += "</div>";
      room += "</div>";
